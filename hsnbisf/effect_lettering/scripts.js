...
},{"ev-emitter":2}],5:[function(require,module,exports){
/*!
...
( function(window) {
    ...
    // basic element class (necessary to apply effects)
    var ISF_El = require('elements/basic/script.js');

    // effects
    var ISF_X_Lettering = require('effects/letters/script.js');

    // elements
    var ISF_El_SVGSnob = require('elements/svgsnob/script.js');
    ...
    function InteractivePage(el) {
        ...
        this.effects = {};
        this.effects.x_svgfilter = ISF_X_SVGFilters;
    ...
    InteractivePage.prototype.init = function() {
        ...
        this.elems.basic = [];
        Array.from( this.DOM.story.querySelectorAll( ".isf-el" ) ).forEach(function(item) {
            self.elems.basic.push(new ISF_El(item, controller));
        });

        this.elems.svgsnobs = [];
        Array.from( this.DOM.story.querySelectorAll( ".isf-el_svgsnob" ) ).forEach(function(item) {
            self.elems.svgsnobs.push(new ISF_El_SVGSnob(item, controller));
        });
...
},{"classes.js":8,"effects/letters/script.js":13,"elements/basic/script.js":14,"elements/svgsnob/script.js":15,"gsap":3,"helpers/removebadthings.js":12,"imagesLoaded":4,"scrollmagic":5,"scrollmagic/scrollmagic/uncompressed/plugins/animation.gsap":6}],8:[function(require,module,exports){
...
TextFx.prototype.animate = function(direction, effect, callback) {
	...
	this.DOM.lines.forEach(function(ln, index) {
		setTimeout(function(){
			TweenMax.staggerFromTo(ln.querySelectorAll("span"), 1,
...
},{"charming":1,"helpers/lineeq.js":10,"scrollmagic":5}],14:[function(require,module,exports){
...
},{"classes.js":8,"scrollmagic":5}],15:[function(require,module,exports){

// AN ELEMENT IMPLEMENTING SVG FILTERS OVER AN OBJECT,
...
function ISF_SVGSnob(el, controller, options) {
	...
	this.DOM = {el: el};
	this.svgFilters = {
		feDMap : this.DOM.el.querySelector( 'feDisplacementMap' ),
		feGBlur : this.DOM.el.querySelector( 'feGaussianBlur' ),
		feTurb : this.DOM.el.querySelector( 'feTurbulence' )
	};


	//starting value for mousepos
	this.lastMousePos = {x: window.innerWidth/2, y: window.innerHeight/2};

	this.fe = { el: this.DOM.el.querySelector(".fe-info") };
	this.fe.property = this.fe.el.querySelector(".fe").innerHTML;
	this.fe.attrib = this.fe.el.querySelector(".fe-attr").innerHTML;
	this.fe.bez = [];
	var self = this;
	Array.from( this.DOM.el.querySelectorAll(".fe-bezier span") ).forEach( function(el, ind) {
		self.fe.bez.push( Number(el.innerHTML) );
	});
	this.svgSnobTrigger = this.DOM.el.querySelector( ".svg-snob__trigger" );
	this.trig = this.DOM.el.dataset.trig;

	this.init();
}

ISF_SVGSnob.prototype.init = function(){

	var self = this;
	if( this.trig === 'mouse' ) {
		this.DOM.el.addEventListener( 'mousemove', function(e) {

			// we care about the mousepos relative to the screen, not the entire document
			var mpos = {
				x: e.clientX,
				y: e.clientY
			};
			...
			// mousespeed effect
			if( self.svgFilters.feDMap !== null ) {
				TweenMax.to( self.svgFilters.feDMap.scale, 1, {
					baseVal : mapRange( mouseDistance(self.lastMousePos.x, mpos.x, self.lastMousePos.y, mpos.y), 0, 120, 0, 250 )
				});
			}
			if( self.svgFilters.feGBlur !== null ) {
				TweenMax.to( self.svgFilters.feGBlur.stdDeviationX, 1, {
					baseVal : mapRange( mouseDistance(self.lastMousePos.x, mpos.x, self.lastMousePos.y, mpos.y), 0, 120, 3, 20 )
				});
				TweenMax.to( self.svgFilters.feGBlur.stdDeviationY, 1, {
					baseVal : mapRange( mouseDistance(self.lastMousePos.x, mpos.x, self.lastMousePos.y, mpos.y), 0, 120, 3, 20 )
				});
			}

			self.lastMousePos.x = mpos.x;
			self.lastMousePos.y = mpos.y;
		});
	} else {
		this.svgSnobTrigger.addEventListener( 'click', function(e) {
			e.preventDefault();
			TweenMax.to( self.svgFilters[self.fe.property][self.fe.attrib], 2, {
				bezier:[
					{ baseVal: self.fe.bez[1] },
					{ baseVal: self.fe.bez[2] }
				],
				ease: Power3.easeIn
			});
		});
...
module.exports = ISF_SVGSnob;

},{"classes.js":8,"helpers/getmousepos.js":9,"helpers/maprange.js":11,"scrollmagic":5}]},{},[7]);
