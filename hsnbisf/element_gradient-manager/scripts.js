...
},{"gsap/umd/TweenLite":3}],3:[function(require,module,exports){
(function (global){
/*!
 * VERSION: 2.0.2
 * DATE: 2018-08-27
...
},{"ev-emitter":1}],6:[function(require,module,exports){
/*!
...
( function(window) {
    ...
    // elements
    var ISF_GradientManager = require('elements/gradient-manager/script.js');
    ...
    InteractivePage.prototype.init = function() {
        ...
        // init gradient manager
        var colorsArray = ['red 5%', 'pink 30%'];
        this.gradientManager = new ISF_GradientManager(this.DOM.story.querySelector( ".js--gradient-manager" ), colorsArray);
        this.gradientManager.step(['red 5%', 'pink 60%']);

        setTimeout(function(){
            self.gradientManager.step(['red 50%', 'pink 80%'], 3);
        }, 200);
        setTimeout(function(){
            self.gradientManager.step(['blue 0%', 'pink 30%'], 3);
        }, 3000);
    };
...
},{"classes.js":9,"elements/gradient-manager/script.js":11,"gsap":4,"helpers/removebadthings.js":10,"imagesLoaded":5,"scrollmagic":6,"scrollmagic/scrollmagic/uncompressed/plugins/animation.gsap":7}],9:[function(require,module,exports){
...
require('scrollmagic/scrollmagic/uncompressed/plugins/animation.gsap');
var gsap = require("gsap");
require('gsap/umd/ColorPropsPlugin.js');
...
function GradientManager(el, colorsArray, deg){

    this.el = el;
    this.deg = deg || '90deg';
    this.colorsObject = this.convertColorsArrayToObject(colorsArray);
    console.log(this);
    this.init();
}

GradientManager.prototype.init = function(){
    this.setColors(); // set initial colors
};

GradientManager.prototype.setColors = function(el){

    var self = el || this; // necessary when calling from within the step function, which sends self as an arg

    var colorEntries = Object.entries(self.colorsObject);
    var colorString = ''; // convert values to string

    colorEntries.forEach(function(col, ind) {
        if( ind !== colorEntries.length - 1 ) {
            colorString += ( ind !== colorEntries.length - 2 ) ? (col[1] + ', ') : (col[1] + ' ');
        }
    });

    if(isChrome){
        TweenLite.set(self.el, {backgroundImage:"-webkit-linear-gradient(" + self.deg + "," + colorString + ")"});
    } else {
        TweenLite.set(self.el, {background:"linear-gradient(" + self.deg + "," + colorString + ")"});
    }
};

GradientManager.prototype.convertColorsArrayToObject = function(colorsArray){

    var colorsObject = {};

    colorsArray.forEach(function(col, ind) {
        colorsObject['col' + ind] = col;
    });

    return colorsObject;
};

GradientManager.prototype.step = function(colorsArray, speed){

    var self = this;

    var colorsObject = this.convertColorsArrayToObject(colorsArray);

    TweenMax.to(this.colorsObject, speed, {
        colorProps: colorsObject,
        onUpdate: self.setColors,
        onUpdateParams:[ self ]
    });
};
...
},{"gsap":4,"gsap/umd/ColorPropsPlugin.js":2,"scrollmagic/scrollmagic/uncompressed/plugins/animation.gsap":7}]},{},[8]);
