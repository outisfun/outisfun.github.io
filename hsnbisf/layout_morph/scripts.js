...
( function(window) {
    ...
    ///// FRAMEWORK LAYOUTS /////
    var ISF_Layout_Morph = require('layouts/morph/script.js');
    ...
    InteractivePage.prototype.init = function() {
        ...
        Array.from( this.DOM.story.querySelectorAll( CLASSES.MORPH.EL ) ).forEach(function(item) {
            self.layouts.morphs.push(new ISF_Layout_Morph(item, controller));
        });
...
},{"classes.js":10,"gsap":4,"helpers/removebadthings.js":12,"imagesLoaded":5,"layouts/morph/script.js":13,"scrollmagic":6,"scrollmagic/scrollmagic/uncompressed/plugins/animation.gsap":7}],10:[function(require,module,exports){
...
function ISF_Layout_Morph(el, controller) {

	this.controller = controller;

	this.DOM = {el: el};
	this.DOM.svg = this.DOM.el.querySelector( CLASSES.MORPH.SVG );
	this.DOM.img = this.DOM.el.querySelector( CLASSES.MORPH.IMG );
	this.DOM.steps = this.DOM.el.querySelectorAll( CLASSES.MORPH.STEP );

	this.defs =  document.querySelector(".isf-svg__defs");
	this.clipPath = this.defs.querySelector(".isf-svg__clip-path");
    ...
    // blob vars
    this.blobComplexity = 0.6; // 0 - 1
    this.blobContrast = 0.4; // 0 - 1

	this.init();
}

ISF_Layout_Morph.prototype.init = function() {

    this.generateBlobs();
    this.assignPathsToSteps();
    this.fixHeader();
    this.activateSteps();

};

ISF_Layout_Morph.prototype.generateBlobs = function() {

    // setting max blob size
    var blobSize = (window.innerHeight > window.innerWidth) ? window.innerWidth : window.innerHeight;

    var self = this;
    // blobs num = steps num + 1 for the first step
    for( var i = 0; i <= this.DOM.steps.length; i += 1) {

        var blob = blobs({
            size: blobSize,
            complexity: self.blobComplexity,
            contrast: self.blobContrast,
            color: 'none',
            stroke: {
                width: 1,
                color: '#ff0000'
            ...
            guides: false,
            seed: '12' + i + i*2 // or anything else, really.
        });
        ...
        var points = blobEl.getAttribute("d");
        newPath.setAttribute("d", points); //Set path's data
        newPath.style.stroke = "#fff"; //Set stroke colour
        newPath.style.strokeWidth = "1px"; //Set stroke width
        newPath.style.fill = "none"; //Set stroke width

        // append to defs to use in morphing
        this.clipPath.innerHTML += blobG.innerHTML;

        // append to svg el in the viewer - maybe you can do sth interesting with it?
        this.DOM.svg.querySelector("svg").appendChild(newPath);
    }
};

ISF_Layout_Morph.prototype.assignPathsToSteps = function() {

    var self = this;
    // assign the newly created blobs to steps
    Array.from(this.clipPath.querySelectorAll("path")).forEach(function(pth, i) {

         pth.classList.add("isf-svg__path");
         pth.classList.add("isf-svg__path--" + i);
         self.paths.push(pth);

         var coords = pth.getAttribute("d");
         self.pathCoords.push(coords);
    });
};

ISF_Layout_Morph.prototype.fixHeader = function() {
    ...
    this.scene = new ScrollMagic.Scene({
        ...
        .on( 'end', function(e) {
            if( e.scrollDirection === 'FORWARD' ) {
                self.DOM.el.classList.add('is--bottom');
            } else {
                self.DOM.el.classList.remove('is--bottom');
            }
        })
        .addTo( this.controller );
};

ISF_Layout_Morph.prototype.fixSteps = function() {
    var self = this;
    // setup page steps
    this.DOM.steps.forEach(function(step, index) {
        var stepScene = new ScrollMagic.Scene({
            ...
            .on('enter', function() {
                self.morphBlob( index );
            })
            .addTo(self.controller);
    });
};

ISF_Layout_Morph.prototype.morphBlob = function(to) {

	var self = this;

	var morphBlob = Snap.select('.isf-svg__path--0');

	var startPoints = this.paths[to].getAttribute('d');
	var finPoints = self.paths[to + 1].getAttribute('d');

	var finBlob = function(){
		morphBlob.animate({ d: finPoints }, 1000, mina.easeout, function() {
			self.currentPath = to;
		});
	};
...
module.exports = ISF_Layout_Morph;

},{"blobs":1,"classes.js":10,"helpers/getmousepos.js":11,"scrollmagic":6,"snapsvg":8}]},{},[9]);
